...
- `"상태"를 객체 분할의 중심 축으로 한다면 ?`  
  - 구현에 관한 세부사항이 객체의 인터페이스에 들어간다.
  - 캡슐화의 원칙이 무너진다.
  - 상태 변경은 인터페이스의 변경이 되어가고 인터페이스에 의존하는 모든 객체에게 영향이 퍼진다.
...
- 캡슐화
  - `상태와 행동을 하나의 객체 안에 모으는 이유 ?`
    - 객체의 내부 구현을 감추기 위해
    - 변경이 전체 시스템에 영향을 끼치지 않도록 파급 효과를 조절
  - `구현`
    - 변경될 가능성이 높은 부분
  - `인터페이스`
    - 상대적으로 안정적인 부분
  - `객체지향 설계의 중요한 원리`
    - `불안정한 구현 세부사항을 인터페이스 뒤로 캡슐화하는 것`
- 캡슐화 위반
  - `자신의 정보를 파라미터로 받는 것`
    - 내부 구현의 변경이 외부로 퍼져나가는 파급 효과
    - 캡슐화가 부족하다는 증거이다.
  - `시그니처를 통한 객체 내부 상태 노출`
- `진정한 캡슐화`
  - `변할 수 있는 어떤 것이라도 감추는 것을 의미한다.`